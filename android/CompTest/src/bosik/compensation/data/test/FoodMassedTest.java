...
public class FoodMassedTest extends TestCase
{
	...
	{
		// нормальный тест
		food.setMass(0.0);		assertEquals(0.0, food.getMass());
		food.setMass(0.01);		assertEquals(0.01, food.getMass());
		food.setMass(100.0);	assertEquals(100.0, food.getMass());
		food.setMass(999.5);	assertEquals(999.5, food.getMass());
		
		// краш-тест
		try { food.setMass(-0.01);	fail(); } catch (Exception e) {}
		try { food.setMass(-100);	fail(); } catch (Exception e) {}
	}
	...
	public void testGetAbs()
	{
		...
		food.setRelProts(1);	assertEquals(0.1, food.getProts());
		food.setRelFats(2); 	assertEquals(0.2, food.getFats());
		food.setRelCarbs(3);	assertEquals(0.3, food.getCarbs());
		food.setRelValue(4);	assertEquals(0.4, food.getValue());
		
		food.setMass(1000);
		
		food.setRelProts(2);	assertEquals(20.0, food.getProts());
		food.setRelFats(4); 	assertEquals(40.0, food.getFats());
		food.setRelCarbs(6);	assertEquals(60.0, food.getCarbs());
		food.setRelValue(8);	assertEquals(80.0, food.getValue());
