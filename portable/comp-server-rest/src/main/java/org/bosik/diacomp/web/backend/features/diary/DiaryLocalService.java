...
public class DiaryLocalService implements UserDataService<DiaryRecord>
{
	...
	public List<Versioned<DiaryRecord>> findPeriod(int userId, Date startTime, Date endTime, boolean includeRemoved)
	{
		if (includeRemoved)
		{
			return convert(repository.findByUserIdAndTimeStampBetweenOrderByTimeCache(userId, startTime, endTime));
		}
		else
		{
			return convert(repository.findByUserIdAndTimeStampBetweenAndDeletedIsFalseOrderByTimeCache(userId, startTime, endTime));
		}
	}
