...
public class FoodUserRestTest
{
	...
	{
		// given
		when(userLocalService.count(anyInt())).thenReturn(42);
	...
	{
		// given
		when(userLocalService.count(anyInt(), eq("ea"))).thenReturn(12);
	...
	public void findById_all() throws Exception
	{
		...
		when(userLocalService.findAll(anyInt(), eq(true))).thenReturn(data);
	...
	public void findById_prefix() throws Exception
	{
		// given
		List<Versioned<FoodItem>> data = buildDemoData();
		when(userLocalService.findByIdPrefix(anyInt(), eq("ff"))).thenReturn(data);
	...
	public void findById_single() throws Exception
	{
		...
		when(userLocalService.findById(anyInt(), eq("542a8a10ef1a41ecb9338dbeb4a931fa"))).thenReturn(data);
	...
	{
		// given
		when(userLocalService.findById(anyInt(), eq("542a8a10ef1a41ecb9338dbeb4a931fa"))).thenReturn(null);
	...
	public void findAll_nonDeleted() throws Exception
	{
		// given
		List<Versioned<FoodItem>> data = buildDemoData();
		when(userLocalService.findAll(anyInt(), eq(false))).thenReturn(data);
	...
	public void findAny() throws Exception
	{
		...
		when(userLocalService.findAny(anyInt(), eq(query))).thenReturn(data);
	...
	public void findChanged() throws Exception
	{
		...
		Date since = new Date();
		List<Versioned<FoodItem>> data = buildDemoData();
		when(userLocalService.findChanged(anyInt(), any())).thenReturn(data);
	...
	public void getHash_root() throws Exception
	{
		...
		when(userLocalService.getHashTree(eq(userId))).thenReturn(hashTree);
	...
	public void getHashChildren_badRequest_tooLong() throws Exception
	{
		...
		final int userId = 17;
		when(userInfoService.getCurrentUserId()).thenReturn(userId);
		when(userLocalService.getHashTree(eq(userId))).thenReturn(new MemoryMerkleTree3(new HashMap<>()));
	...
	public void save() throws Exception
	{
		...
		verify(userLocalService).save(eq(userId), any());
	}
